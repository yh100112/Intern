...
void send_file(int* arg,char* file_name,char* file_name2){
  ...
  if(fp == NULL){error_handling("file이 존재하지 않습니다.\n");}
  fseek(fp,0,SEEK_END);
  p = ftell(fp);
  fseek(fp,0,SEEK_SET);
  file_size = htonl(p); 
  ...
  n = strlen(file_name);
  length = htonl(n);
  write(serv_sock,&length,sizeof(int)); 
  write(serv_sock,file_name,strlen(file_name));
  write(serv_sock,&file_size,sizeof(int)); 

  fread(file_comment,sizeof(char),file_size,fp);
  write(serv_sock,&file_comment,p); 
